...
export async function searchCode(
  ...
  try {
    ...
    const lines = [`Found ${results.length} match${results.length === 1 ? '' : 'es'} for "${args.query}":\n`];
    ...
    results.forEach((result, index) => {
      ...
      if (result.node.startLine) {
        lines.push(`   📍 Lines ${result.node.startLine}-${result.node.endLine || result.node.startLine}`);
      }
